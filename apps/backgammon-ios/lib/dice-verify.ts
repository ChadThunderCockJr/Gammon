...
async function deriveDice(
  ...
  const data = encoder.encode(
    drandRandomness + playerWhite + playerBlack + String(turnNumber),
  );
...
export async function verifyDiceRoll(params: {
  ...
  if (!sigValid) {
    return {
      valid: false,
      details: `BLS signature verification failed for drand round ${drandRound}`,
    };
  }

  // 2. Verify randomness = SHA256(signature)
  ...
  if (expectedRandomness !== drandRandomness) {
    return {
      valid: false,
      details: `Randomness mismatch: expected ${expectedRandomness}, got ${drandRandomness}`,
    };
  }

  // 3. Verify dice derivation
  const expectedDice = await deriveDice(
    drandRandomness,
    playerWhite,
    playerBlack,
    turnNumber,
  ...
  return {
    valid: true,
    details: "BLS signature verified, randomness derived correctly, dice match",
  };
}
